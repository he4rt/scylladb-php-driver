...
#include "src/Collections/Tuple.h"
...
int php_driver_value(const CassValue *value, const CassDataType *data_type, zval *out) {
  ...
  switch (type) {
    ...
    case CASS_VALUE_TYPE_INT:
      ASSERT_SUCCESS_BLOCK(cass_value_get_int32(value, &v_int_32), zval_ptr_dtor(out);
                           return FAILURE;);
    ...
    case CASS_VALUE_TYPE_TIMESTAMP:
      ...
      ASSERT_SUCCESS_BLOCK(cass_value_get_int64(value, &timestamp->timestamp), zval_ptr_dtor(out);
                           return FAILURE;);
      break;
    case CASS_VALUE_TYPE_DATE:
      object_init_ex(out, php_scylladb_date_ce);
      date = Z_SCYLLADB_DATE_P(out);
      ASSERT_SUCCESS_BLOCK(cass_value_get_uint32(value, &date->date), zval_ptr_dtor(out);
                           return FAILURE;);
      break;
    case CASS_VALUE_TYPE_TIME:
      object_init_ex(out, php_scylladb_time_ce);
      time = Z_SCYLLADB_TIME_P(out);
      ASSERT_SUCCESS_BLOCK(cass_value_get_int64(value, &time->time), zval_ptr_dtor(out);
                           return FAILURE;);
      break;
    case CASS_VALUE_TYPE_BLOB:
      object_init_ex(out, php_driver_blob_ce);
      blob = PHP_DRIVER_GET_BLOB(out);
      ASSERT_SUCCESS_BLOCK(cass_value_get_bytes(value, &v_bytes, &v_bytes_len), zval_ptr_dtor(out);
                           return FAILURE;);
    ...
    case CASS_VALUE_TYPE_UUID:
      ...
      ASSERT_SUCCESS_BLOCK(cass_value_get_uuid(value, &uuid->uuid), zval_ptr_dtor(out);
                           return FAILURE;);
      break;
    ...
    case CASS_VALUE_TYPE_BOOLEAN:
      ASSERT_SUCCESS_BLOCK(cass_value_get_bool(value, &v_boolean), zval_ptr_dtor(out);
                           return FAILURE;);
      if (v_boolean) {
        ZVAL_TRUE(out);
      } else {
    ...
    case CASS_VALUE_TYPE_INET:
      ...
      ASSERT_SUCCESS_BLOCK(cass_value_get_inet(value, &inet->inet), zval_ptr_dtor(out);
                           return FAILURE;);
      break;
    case CASS_VALUE_TYPE_DECIMAL:
      ...
      ASSERT_SUCCESS_BLOCK(
          cass_value_get_decimal(value, &v_decimal, &v_decimal_len, &v_decimal_scale),
          zval_ptr_dtor(out);
          return FAILURE;);
    ...
    case CASS_VALUE_TYPE_DURATION:
      ...
      ASSERT_SUCCESS_BLOCK(
          cass_value_get_duration(value, &duration->months, &duration->days, &duration->nanos),
          zval_ptr_dtor(out);
          return FAILURE;);
      break;
    case CASS_VALUE_TYPE_DOUBLE:
      ASSERT_SUCCESS_BLOCK(cass_value_get_double(value, &v_double), zval_ptr_dtor(out);
                           return FAILURE;);
    ...
    case CASS_VALUE_TYPE_MAP:
      ...
      while (cass_iterator_next(iterator)) {
        ...
        if (php_driver_value(cass_iterator_get_map_key(iterator), primary_type, &k) == FAILURE ||
            php_driver_value(cass_iterator_get_map_value(iterator), secondary_type, &v) ==
                FAILURE) {
          cass_iterator_free(iterator);
          zval_ptr_dtor(out);
          return FAILURE;
...
int php_driver_get_keyspace_field(const CassKeyspaceMeta *metadata, const char *field_name,
                                  zval *out) {
...
int php_driver_get_result(const CassResult *result, zval *out) {
  ...
  while (cass_iterator_next(iterator)) {
    ...
    for (i = 0; i < columns; i++) {
      ...
      PHP5TO7_ADD_ASSOC_ZVAL_EX(&row, column_names[i], strlen(column_names[i]) + 1, &value);
    }
