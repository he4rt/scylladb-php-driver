...
static inline unsigned double_hash(cass_double_t value) {
  return php_driver_bigint_hash(double_to_bits(value));
}

PHP_DRIVER_ALWAYS_INLINE uint32_t php_driver_value_hash(zval* zvalue) {
...
uint32_t php_driver_mpz_hash(uint32_t seed, mpz_t n) {
  mp_size_t i;
