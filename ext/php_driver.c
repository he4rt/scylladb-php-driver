...
static const logging logging_levels[6] = {
  ...
  { "INFO", sizeof("INFO") - 1, CASS_LOG_INFO },
  { "DEBUG", sizeof("DEBUG") - 1, CASS_LOG_DEBUG },
...
PHP_INI_MH(OnUpdateLogLevel)
{
  ...
  FOREACH_STATIC_ARRAY(logging_levels, logging, level)
  {
    if (level.str_level_size == ZSTR_LEN(new_value) && strcmp((char*) ZSTR_VAL(new_value), (char*) level.str_level) == 0) {
      cass_log_set_level(level.level);
      return SUCCESS;
    }
  }
  FOREACH_STATIC_ARRAY_END();
