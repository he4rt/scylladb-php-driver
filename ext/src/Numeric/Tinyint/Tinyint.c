...
PHP_METHOD(Tinyint, sub)
{
  ...
  if (Z_TYPE_P(difference) == IS_OBJECT && instanceof_function(Z_OBJCE_P(difference), php_driver_tinyint_ce TSRMLS_CC)) {
    php_driver_numeric* self    = PHP_DRIVER_GET_NUMERIC(getThis());
...
PHP_METHOD(Tinyint, mul)
{
  ...
  if (Z_TYPE_P(multiplier) == IS_OBJECT && instanceof_function(Z_OBJCE_P(multiplier), php_driver_tinyint_ce TSRMLS_CC)) {
    php_driver_numeric* self    = PHP_DRIVER_GET_NUMERIC(getThis());
