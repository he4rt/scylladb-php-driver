...
PHP_METHOD(BatchStatement, add)
{
  ...
  if (Z_TYPE_P(statement) != IS_STRING && (Z_TYPE_P(statement) != IS_OBJECT || (!instanceof_function(Z_OBJCE_P(statement), php_driver_simple_statement_ce) && !instanceof_function(Z_OBJCE_P(statement), php_driver_prepared_statement_ce)))) {
