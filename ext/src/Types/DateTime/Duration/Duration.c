...
static void
to_string(zval* result, cass_int64_t value)
{
  char* string;
          ...
          cass_int64_t* destination)
{
  if (Z_TYPE_P(value) == IS_LONG) {
    ...
    *destination = bigint_value;
    ...
    throw_invalid_argument(value, param_name, "a long, a double, a numeric string or a " PHP_DRIVER_NAMESPACE "\\Bigint" TSRMLS_CC);
    return 0;
  }
  return 1;
}
...
char*
php_driver_duration_to_string(php_driver_duration* duration)
{
...
php_driver_duration_init(INTERNAL_FUNCTION_PARAMETERS)
{
  ...
  if (!(self->months <= 0 && self->days <= 0 && self->nanos <= 0) && !(self->months >= 0 && self->days >= 0 && self->nanos >= 0)) {
...
void
php_driver_define_Duration(TSRMLS_D)
{
  zend_class_entry ce;
