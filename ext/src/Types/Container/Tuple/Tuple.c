...
PHP_METHOD(Tuple, __construct)
{
  ...
  PHP5TO7_ZEND_HASH_FOREACH_VAL(types, current) {
    ...
    } else if (Z_TYPE_P(sub_type) == IS_OBJECT && instanceof_function(Z_OBJCE_P(sub_type), php_driver_type_ce)) {
...
PHP_METHOD(Tuple, set)
{
  ...
  if (!PHP5TO7_ZEND_HASH_INDEX_FIND(&type->data.tuple.types, index, sub_type) || !php_driver_validate_object(value, PHP5TO7_ZVAL_MAYBE_DEREF(sub_type))) {
    return;
  }
