...
php_driver_decimal_init(php_driver_numeric* self, zval* value)
{
  ...
  case IS_OBJECT: {
    if (instanceof_function(Z_OBJCE_P(value), php_driver_decimal_ce)) {
      php_driver_numeric* decimal = PHP_DRIVER_NUMERIC_ZVAL_TO_OBJECT(value);
...
ZEND_METHOD(Cassandra_Decimal, add)
{
  ...
  php_driver_numeric* self    = PHP_DRIVER_NUMERIC_THIS();
  php_driver_numeric* decimal = PHP_DRIVER_NUMERIC_ZVAL_TO_OBJECT(num);
...
ZEND_METHOD(Cassandra_Decimal, sqrt)
{
  ...
  object_init_ex(return_value, php_driver_decimal_ce);
  php_driver_numeric *result = PHP_DRIVER_NUMERIC_OBJECT(return_value);
