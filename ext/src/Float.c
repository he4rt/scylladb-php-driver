...
php_driver_float_init(INTERNAL_FUNCTION_PARAMETERS)
{
  ...
  } else if (Z_TYPE_P(value) == IS_OBJECT && instanceof_function(Z_OBJCE_P(value), php_driver_float_ce TSRMLS_CC)) {
...
PHP_METHOD(Float, add)
{
  ...
  if (Z_TYPE_P(num) == IS_OBJECT && instanceof_function(Z_OBJCE_P(num), php_driver_float_ce TSRMLS_CC)) {
    php_driver_numeric* self = PHP_DRIVER_GET_NUMERIC(getThis());
...
void
php_driver_define_Float(TSRMLS_D)
{
  zend_class_entry ce;
