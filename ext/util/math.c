...
php_driver_parse_float(char* in, int in_len, cass_float_t* number)
{
  ...
  *number = (cass_float_t) strtof(in, &end);
  ...
  if (errno == ERANGE) {
    zend_throw_exception_ex(spl_ce_RangeException, 0, "Value is too small or too big for float: '%s'", in);
    return 0;
  }

  if (errno || end == in) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Invalid float value: '%s'", in);
    return 0;
  }

  if (end != &in[in_len]) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Invalid characters were found in value: '%s'", in);
    return 0;
  }
...
php_driver_parse_double(char* in, int in_len, cass_double_t* number)
{
  ...
  *number = (cass_double_t) strtod(in, &end);
  ...
  if (errno == ERANGE) {
    zend_throw_exception_ex(spl_ce_RangeException, 0, "Value is too small or too big for double: '%s'", in);
    return 0;
  }

  if (errno || end == in) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Invalid double value: '%s'", in);
    return 0;
  }
...
php_driver_parse_int(char* in, int in_len, cass_int32_t* number)
{
  ...
  if (errno || end == &in[pos]) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Invalid integer value: '%s'", in);
    return 0;
  }
...
php_driver_parse_decimal(char* in, int in_len, mpz_t* number, long* scale)
{
  ...
  while (point < in_len) {
    ...
    if (c == '.') {
      ...
      if (dot != -1) {
        zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Multiple '.' (dots) in the number '%s'", in);
        return 0;
      }
     ...
     */
      ...
      zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Unrecognized character '%c' at position %d", c, point);
      return 0;
    }
  ...
  if (out_len == 0) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "No digits seen in value: '%s'", in);
    return 0;
  }

  if (mpz_set_str(*number, out, 10) == -1) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Unable to extract integer part of decimal value: '%s', %s", in, out);
    efree(out);
    return 0;
  }
  ...
  if (point < in_len) {
     ...
     */
    if (point >= in_len) {
      zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "No exponent following e or E in value: '%s'", in);
      return 0;
    }

    if (!sscanf(&in[point], "%d", &diff)) {
      zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0, "Malformed exponent in value: '%s'", in);
      return 0;
    }
