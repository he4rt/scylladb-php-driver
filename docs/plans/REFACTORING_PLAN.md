...
- [~] **Drop `php_driver_ref` indirection** — in progress.
  - [x] **Part 1: SchemaMeta** (2026-05-22, commit `b52baecf` on PR #129). DefaultSchema now owns `const CassSchemaMeta *schema_meta` directly; the 7 sub-objects (Keyspace, Table, Column, Index, MaterializedView, Function, Aggregate) hold a `zval schema` to the parent DefaultSchema PHP object. Zend's normal zval refcounting handles lifetime. ~25 php_driver_ref callsites removed.
  - [ ] **Part 2: Session/Statement/Result** — design ready, scope TBD. Plan:
