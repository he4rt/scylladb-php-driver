...
- [x] **Drop `php_driver_ref` indirection** — done (2026-05-22). Zero `php_driver_ref` references remain anywhere in `src/`, `include/`, `util/`. `util/ref.{h,cpp}` deleted from disk and from `util/CMakeLists.txt`.
  - **Part 1: SchemaMeta** (commit `b52baecf`). DefaultSchema now owns `const CassSchemaMeta *schema_meta` directly; the 7 sub-objects (Keyspace, Table, Column, Index, MaterializedView, Function, Aggregate) hold a `zval schema` to the parent DefaultSchema PHP object. Zend's normal zval refcounting handles lifetime.
  - **Part 2: Session/Statement/Result**:
    - Session: direct `CassSession *session` field. Per-request DefaultSession is the owner when `!persist`; the existing `psession` in `EG(persistent_list)` is the owner when `persist`. Other PHP objects (FutureSession during async, FutureRows, Rows) hold a `zval` to DefaultSession (Pattern B).
    - FutureSession-before-resolution owns its CassSession* directly; `get()` transfers ownership to the new DefaultSession.
    - Statement: needs refcounted sharing (nextPageAsync gives the same `CassStatement *` to a sibling FutureRows). Use `zend_resource` with type `le_cass_statement` and destructor `cass_statement_free`. Holders go through `zend_list_addref` / `zend_list_close`.
    - Result: each Rows page has its own `const CassResult *`. Ownership transferred (not shared) on page advance: `rows->result = current->next_result; current->next_result = NULL`. Direct pointer, no resource needed.
    - `pprepared_statement.ref` (back-ref to session): drop entirely. PHP's `persistent_list` cleanup is LIFO, so the prepared-statement entry is destroyed before the session it depends on; the CassFuture inside holds an internal CassSession ref that survives.
- [x] **Pest tests** (2026-05-22). New [tests/Feature/Sessions/PersistentSessionTest.php](tests/Feature/Sessions/PersistentSessionTest.php) covers: cluster cache hit, cluster cache miss (distinct configs), session cache hit, session cache miss (distinct keyspaces), prepared-statement cache hit, prepared-statement cache miss (distinct CQL), broken-future invalidation (unreachable host doesn't poison the cache), and the hash_key UAF regression (drop `$cluster` zval before the session, then `prepare()` — must not segfault). Counters parsed from `phpinfo()` via `persistentCounters()` helper in tests/Support/helpers.php. New `persistentScyllaDbBuilder()` helper for the persistent-mode builder. Tests auto-pick up under the existing Feature suite; CI already runs them against the live ScyllaDB service.
