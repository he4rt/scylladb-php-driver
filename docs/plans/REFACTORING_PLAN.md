...
- [ ] Drop `php_driver_ref` indirection — resources have native refcounting. **Still deferred** — ~120 dereference sites across 14 files + new resource type registrations + non-trivial restructuring of the persistent/non-persistent ownership asymmetry. Now safety-netted by the Pest tests below; can be tackled in a dedicated follow-up with the regression-test surface in place.
