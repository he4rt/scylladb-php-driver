...
zval php_driver_create_column(php_driver_ref *schema, const CassColumnMeta *meta) {
  ...
  if (value) {
    ...
    ASSERT_SUCCESS_BLOCK(cass_value_get_string(value, &validator, &validator_length),
                         zval_ptr_dtor(&result);
                         ZVAL_UNDEF(&result); return result;);

    if (php_driver_parse_column_type(validator, validator_length, &column->reversed,
                                     &column->frozen, &column->type) == FAILURE) {
      zval_ptr_dtor(&result);
      ZVAL_UNDEF(&result);
      return result;
    }
    ...
    if (data_type) {
      ...
      ASSERT_SUCCESS_BLOCK(
          cass_value_get_string(value, &clustering_order, &clustering_order_length),
          zval_ptr_dtor(&result);
          ZVAL_UNDEF(&result); return result;);
      column->reversed = strncmp(clustering_order, "desc", clustering_order_length) == 0 ? 1 : 0;
    }
  }
...
static zend_function_entry php_driver_default_column_methods[] = {
    PHP_ME(DefaultColumn, name, arginfo_none, ZEND_ACC_PUBLIC)
        PHP_ME(DefaultColumn, type, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
            DefaultColumn, isReversed, arginfo_none, ZEND_ACC_PUBLIC | ZEND_ACC_DEPRECATED)
            PHP_ME(DefaultColumn, isStatic, arginfo_none, ZEND_ACC_PUBLIC)
                PHP_ME(DefaultColumn, isFrozen, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                    DefaultColumn, indexName, arginfo_none, ZEND_ACC_PUBLIC | ZEND_ACC_DEPRECATED)
                    PHP_ME(DefaultColumn, indexOptions, arginfo_none,
                           ZEND_ACC_PUBLIC | ZEND_ACC_DEPRECATED) PHP_FE_END};

static zend_object_handlers php_driver_default_column_handlers;

static HashTable *php_driver_type_default_column_gc(
...
static HashTable *php_driver_default_column_properties(
#if PHP_MAJOR_VERSION >= 8
    zend_object *object
#else
    zendObject *object
...
static int php_driver_default_column_compare(zval *obj1, zval *obj2) {
#if PHP_MAJOR_VERSION >= 8
...
static void php_driver_default_column_free(zend_object *object) {
...
static zend_object *php_driver_default_column_new(zend_class_entry *ce) {
  php_driver_column *self = PHP5TO7_ZEND_OBJECT_ECALLOC(column, ce);
...
void php_driver_define_DefaultColumn() {
  ...
  memcpy(&php_driver_default_column_handlers, zend_get_std_object_handlers(),
