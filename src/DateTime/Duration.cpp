...
static int get_param(zval *value, const char *param_name, cass_int64_t min, cass_int64_t max,
  ...
  if (Z_TYPE_P(value) == IS_LONG) {
    ...
    *destination = bigint_value;
    ...
    throw_invalid_argument(value, param_name,
                           "a long, a double, a numeric string or a " PHP_DRIVER_NAMESPACE
                           "\\Bigint");
    return 0;
  }
  return 1;
...
char *php_driver_duration_to_string(php_driver_duration *duration) {
  ...
  if (final_months < 0) final_months = -final_months;
  if (final_days < 0) final_days = -final_days;
  if (final_nanos < 0) final_nanos = -final_nanos;

  spprintf(&rep, 0, "%s%dmo%dd%" PRId64 "ns", is_negative ? "-" : "", final_months, final_days,
           final_nanos);
  return rep;
}
...
PHP_METHOD(Duration, __construct) { php_driver_duration_init(INTERNAL_FUNCTION_PARAM_PASSTHRU); }
            ...
            PHP_ME(Duration, months, arginfo_none, ZEND_ACC_PUBLIC)
                PHP_ME(Duration, days, arginfo_none, ZEND_ACC_PUBLIC)
                    PHP_ME(Duration, nanos, arginfo_none, ZEND_ACC_PUBLIC)
                        PHP_ME(Duration, __toString, arginfo_tostring, ZEND_ACC_PUBLIC) PHP_FE_END};

static zend_object_handlers php_driver_duration_handlers;

static HashTable *php_driver_duration_properties(
...
static int php_driver_duration_compare(zval *obj1, zval *obj2) {
#if PHP_MAJOR_VERSION >= 8
  ...
  if (left->days < right->days) return -1;
...
static void php_driver_duration_free(zend_object *object) {
  php_driver_duration *self = PHP5TO7_ZEND_OBJECT_GET(duration, object);
...
static zend_object *php_driver_duration_new(zend_class_entry *ce) {
...
void php_driver_define_Duration() {
  ...
  memcpy(&php_driver_duration_handlers, zend_get_std_object_handlers(),
         sizeof(zend_object_handlers));
  php_driver_duration_handlers.get_properties = php_driver_duration_properties;
  php_driver_duration_handlers.compare = php_driver_duration_compare;
