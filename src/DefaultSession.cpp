...
static int bind_argument_by_index(CassStatement *statement, size_t index,
                                  zval *value) {
  ...
  if (Z_TYPE_P(value) == IS_STRING)
    CHECK_RESULT(
        cass_statement_bind_string(statement, index, Z_STRVAL_P(value)));
  ...
  if (Z_TYPE_P(value) == IS_OBJECT) {
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_timestamp_ce)) {
      ...
      CHECK_RESULT(
          cass_statement_bind_int64(statement, index, timestamp->timestamp));
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_blob_ce)) {
      ...
      CHECK_RESULT(
          cass_statement_bind_bytes(statement, index, blob->data, blob->size));
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_varint_ce)) {
      ...
      size_t size;
      cass_byte_t *data =
          export_twos_complement(varint->data.varint.value, &size);
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_decimal_ce)) {
      ...
      size_t size;
      cass_byte_t *data = (cass_byte_t *)export_twos_complement(
          decimal->data.decimal.value, &size);
      CassError rc = cass_statement_bind_decimal(statement, index, data, size,
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_duration_ce)) {
      ...
      CHECK_RESULT(cass_statement_bind_duration(
          statement, index, duration->months, duration->days, duration->nanos));
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_user_type_value_ce)) {
      ...
      php_driver_user_type_value *user_type_value =
          PHP_DRIVER_GET_USER_TYPE_VALUE(value);
...
static int bind_argument_by_name(CassStatement *statement, const char *name,
                                 zval *value) {
  if (Z_TYPE_P(value) == IS_NULL) {
  ...
  if (Z_TYPE_P(value) == IS_STRING)
    CHECK_RESULT(
        cass_statement_bind_string_by_name(statement, name, Z_STRVAL_P(value)));

  if (Z_TYPE_P(value) == IS_DOUBLE)
    CHECK_RESULT(
        cass_statement_bind_double_by_name(statement, name, Z_DVAL_P(value)));

  if (Z_TYPE_P(value) == IS_LONG)
    CHECK_RESULT(
        cass_statement_bind_int32_by_name(statement, name, Z_LVAL_P(value)));
  ...
  if (Z_TYPE_P(value) == IS_OBJECT) {
    if (instanceof_function(Z_OBJCE_P(value), php_driver_float_ce)) {
      php_driver_numeric *float_number = PHP_DRIVER_GET_NUMERIC(value);
      CHECK_RESULT(cass_statement_bind_float_by_name(
          statement, name, float_number->data.floating.value));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_bigint_ce)) {
      php_driver_numeric *bigint = PHP_DRIVER_GET_NUMERIC(value);
      CHECK_RESULT(cass_statement_bind_int64_by_name(
          statement, name, bigint->data.bigint.value));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_smallint_ce)) {
      php_driver_numeric *smallint = PHP_DRIVER_GET_NUMERIC(value);
      CHECK_RESULT(cass_statement_bind_int16_by_name(
          statement, name, smallint->data.smallint.value));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_tinyint_ce)) {
      php_driver_numeric *tinyint = PHP_DRIVER_GET_NUMERIC(value);
      CHECK_RESULT(cass_statement_bind_int8_by_name(
          statement, name, tinyint->data.tinyint.value));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_timestamp_ce)) {
      php_driver_timestamp *timestamp = PHP_DRIVER_GET_TIMESTAMP(value);
      CHECK_RESULT(cass_statement_bind_int64_by_name(statement, name,
                                                     timestamp->timestamp));
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_date_ce)) {
      php_driver_date *date = PHP_DRIVER_GET_DATE(value);
      CHECK_RESULT(
          cass_statement_bind_uint32_by_name(statement, name, date->date));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_time_ce)) {
      php_driver_time *time = PHP_DRIVER_GET_TIME(value);
      CHECK_RESULT(
          cass_statement_bind_int64_by_name(statement, name, time->time));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_blob_ce)) {
      php_driver_blob *blob = PHP_DRIVER_GET_BLOB(value);
      CHECK_RESULT(cass_statement_bind_bytes_by_name(statement, name,
                                                     blob->data, blob->size));
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_varint_ce)) {
      ...
      size_t size;
      cass_byte_t *data = (cass_byte_t *)export_twos_complement(
          varint->data.varint.value, &size);
      CassError rc =
          cass_statement_bind_bytes_by_name(statement, name, data, size);
      free(data);
      CHECK_RESULT(rc);
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_decimal_ce)) {
      ...
      cass_byte_t *data = (cass_byte_t *)export_twos_complement(
          decimal->data.decimal.value, &size);
      CassError rc = cass_statement_bind_decimal_by_name(
          statement, name, data, size, decimal->data.decimal.scale);
      free(data);
      CHECK_RESULT(rc);
    }
    ...
    if (instanceof_function(Z_OBJCE_P(value), php_driver_uuid_interface_ce)) {
      php_driver_uuid *uuid = PHP_DRIVER_GET_UUID(value);
      CHECK_RESULT(
          cass_statement_bind_uuid_by_name(statement, name, uuid->uuid));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_inet_ce)) {
      php_driver_inet *inet = PHP_DRIVER_GET_INET(value);
      CHECK_RESULT(
          cass_statement_bind_inet_by_name(statement, name, inet->inet));
    }

    if (instanceof_function(Z_OBJCE_P(value), php_driver_duration_ce)) {
      php_driver_duration *duration = PHP_DRIVER_GET_DURATION(value);
      CHECK_RESULT(cass_statement_bind_duration_by_name(
          statement, name, duration->months, duration->days, duration->nanos));
    }
...
static int bind_arguments(CassStatement *statement, HashTable *arguments) {
  ...
  char *str_key;
  uint str_len;
  PHP5TO7_ZEND_HASH_FOREACH_KEY_VAL(arguments, num_key, str_key, str_len,
                                    current) {
    if (str_key) {
      ...
      rc = bind_argument_by_index(statement, num_key,
                                  PHP5TO7_ZVAL_MAYBE_DEREF(current));
    }
    if (rc == FAILURE)
      break;
...
static CassStatement *create_statement(php_driver_statement *statement,
                                       HashTable *arguments) {
  CassStatement *stmt;
  uint32_t count;
...
static CassBatch *create_batch(php_driver_statement *batch,
  ...
  PHP5TO7_ZEND_HASH_FOREACH_VAL(&batch->data.batch.statements, current) {
    ...
    if (PHP5TO7_Z_TYPE_MAYBE_P(batch_statement_entry->statement) == IS_STRING) {
      ...
      simple_statement.data.simple.cql =
          PHP5TO7_Z_STRVAL_MAYBE_P(batch_statement_entry->statement);
      statement = &simple_statement;
    } else {
      statement = PHP_DRIVER_GET_STATEMENT(
          PHP5TO7_ZVAL_MAYBE_P(batch_statement_entry->statement));
    }
...
create_single(php_driver_statement *statement, HashTable *arguments,
  ...
  if (rc == CASS_OK && serial_consistency >= 0)
    rc = cass_statement_set_serial_consistency(
        stmt, static_cast<CassConsistency>(serial_consistency));
...
PHP_METHOD(DefaultSession, execute) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "z|z", &statement, &options) ==
      FAILURE) {
    return;
  }
  ...
  if (Z_TYPE_P(statement) == IS_STRING) {
    ...
    stmt = &simple_statement;
             ...
             instanceof_function(Z_OBJCE_P(statement),
                                 php_driver_statement_ce)) {
    stmt = PHP_DRIVER_GET_STATEMENT(statement);
  } else {
    INVALID_ARGUMENT(statement,
                     "a string or an instance of " PHP_DRIVER_NAMESPACE
                     "\\Statement");
  }
  ...
  if (self->default_timeout.is_set) {
    ZVAL_DOUBLE(timeout, self->default_timeout.timeout);
  } else {
    ZVAL_NULL(timeout);
  }
  ...
  if (options) {
    if (Z_TYPE_P(options) != IS_ARRAY &&
        (Z_TYPE_P(options) != IS_OBJECT ||
         !instanceof_function(Z_OBJCE_P(options),
                              php_driver_execution_options_ce))) {
      INVALID_ARGUMENT(options, "an instance of " PHP_DRIVER_NAMESPACE
                                "\\ExecutionOptions or an array or null");
    }

    if (Z_TYPE_P(options) == IS_OBJECT) {
      opts = PHP_DRIVER_GET_EXECUTION_OPTIONS(options);
    } else {
      if (php_driver_execution_options_build_local_from_array(
              &local_opts, options) == FAILURE) {
        return;
      }
      opts = &local_opts;
    }
    ...
    if (!PHP5TO7_ZVAL_IS_UNDEF(opts->retry_policy))
      retry_policy = (PHP_DRIVER_GET_RETRY_POLICY(
                          PHP5TO7_ZVAL_MAYBE_P(opts->retry_policy)))
                         ->policy;
  ...
  case PHP_DRIVER_SIMPLE_STATEMENT:
  case PHP_DRIVER_PREPARED_STATEMENT:
    single = create_single(stmt, arguments, consistency, serial_consistency,
                           page_size, paging_state_token,
                           paging_state_token_size, retry_policy, timestamp);
  ...
  case PHP_DRIVER_BATCH_STATEMENT:
    ...
    future =
        cass_session_execute_batch((CassSession *)self->session->data, batch);
    break;
  default:
    INVALID_ARGUMENT(statement, "an instance of " PHP_DRIVER_NAMESPACE
                                "\\SimpleStatement, " PHP_DRIVER_NAMESPACE
                                "\\PreparedStatement or " PHP_DRIVER_NAMESPACE
                                "\\BatchStatement");
    return;
  }
...
PHP_METHOD(DefaultSession, executeAsync) {
  ...
  case PHP_DRIVER_PREPARED_STATEMENT:
    ...
    future_rows->future =
        cass_session_execute((CassSession *)self->session->data, single);
...
static void free_prepared_statement(void *future) {
...
PHP_METHOD(DefaultSession, prepare) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "z|z", &cql, &options) ==
      FAILURE) {
    return;
  }
  ...
  if (self->persist) {
    ...
    hash_key_len = spprintf(&hash_key, 0, "%s:prepared_statement:%s", hash_key,
                            SAFE_STR(self->keyspace));

    if (PHP5TO7_ZEND_HASH_FIND(&EG(persistent_list), hash_key, hash_key_len + 1,
                               le) &&
  ...
  if (future == NULL) {
    ...
    if (php_driver_future_wait_timed(future, timeout) == SUCCESS &&
      ...
      prepared_statement = PHP_DRIVER_GET_STATEMENT(return_value);
      prepared_statement->data.prepared.prepared =
          cass_future_get_prepared(future);

      if (self->persist) {
        pprepared_statement = (php_driver_pprepared_statement *)pecalloc(
            1, sizeof(php_driver_pprepared_statement), 1);
        pprepared_statement->ref =
            php_driver_new_peref(future, free_prepared_statement, 1);
        ...
        PHP5TO7_ZEND_HASH_UPDATE(&EG(persistent_list), hash_key,
                                 hash_key_len + 1, &resource,
  ...
  if (self->persist) {
    if (php_driver_future_is_error(future) == FAILURE) {
      (void)PHP5TO7_ZEND_HASH_DEL(&EG(persistent_list), hash_key,
                                  hash_key_len + 1);
    }
    efree(hash_key);
  } else {
...
PHP_METHOD(DefaultSession, metrics) {
  ...
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "min", metrics.requests.min);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "max", metrics.requests.max);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "mean", metrics.requests.mean);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "stddev",
                 metrics.requests.stddev);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "median",
                 metrics.requests.median);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "p75",
                 metrics.requests.percentile_75th);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "p95",
                 metrics.requests.percentile_95th);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "p98",
                 metrics.requests.percentile_98th);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "p99",
                 metrics.requests.percentile_99th);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(requests), "p999",
                 metrics.requests.percentile_999th);
  add_assoc_double(PHP5TO7_ZVAL_MAYBE_P(requests), "mean_rate",
                   metrics.requests.mean_rate);
  add_assoc_double(PHP5TO7_ZVAL_MAYBE_P(requests), "m1_rate",
                   metrics.requests.one_minute_rate);
  add_assoc_double(PHP5TO7_ZVAL_MAYBE_P(requests), "m5_rate",
                   metrics.requests.five_minute_rate);
  add_assoc_double(PHP5TO7_ZVAL_MAYBE_P(requests), "m15_rate",
                   metrics.requests.fifteen_minute_rate);

  PHP5TO7_ZVAL_MAYBE_MAKE(stats);
  array_init(PHP5TO7_ZVAL_MAYBE_P(stats));
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(stats), "total_connections",
                 metrics.stats.total_connections);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(stats), "available_connections",
  ...
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(stats), "exceeded_write_bytes_water_mark",
                 metrics.stats.exceeded_write_bytes_water_mark);

  PHP5TO7_ZVAL_MAYBE_MAKE(errors);
  array_init(PHP5TO7_ZVAL_MAYBE_P(errors));
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(errors), "connection_timeouts",
                 metrics.errors.connection_timeouts);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(errors), "pending_request_timeouts",
                 metrics.errors.pending_request_timeouts);
  add_assoc_long(PHP5TO7_ZVAL_MAYBE_P(errors), "request_timeouts",
    ...
    PHP_ME(DefaultSession, execute, arginfo_execute, ZEND_ACC_PUBLIC)
        PHP_ME(DefaultSession, executeAsync, arginfo_execute, ZEND_ACC_PUBLIC)
            PHP_ME(DefaultSession, prepare, arginfo_prepare, ZEND_ACC_PUBLIC)
                PHP_ME(DefaultSession, prepareAsync, arginfo_prepare,
                       ZEND_ACC_PUBLIC) PHP_ME(DefaultSession, close,
                                               arginfo_timeout, ZEND_ACC_PUBLIC)
                    PHP_ME(DefaultSession, closeAsync, arginfo_none,
                           ZEND_ACC_PUBLIC)
                        PHP_ME(DefaultSession, metrics, arginfo_none,
                               ZEND_ACC_PUBLIC)
                            PHP_ME(DefaultSession, schema, arginfo_none,
...
static void php_driver_default_session_free(php5to7_zend_object_free *object) {
...
void php_driver_define_DefaultSession() {
  ...
  INIT_CLASS_ENTRY(ce, PHP_DRIVER_NAMESPACE "\\DefaultSession",
                   php_driver_default_session_methods);
  php_driver_default_session_ce = zend_register_internal_class(&ce);
  zend_class_implements(php_driver_default_session_ce, 1,
                        php_driver_session_ce);
  ...
  php_driver_default_session_handlers.get_properties =
      php_driver_default_session_properties;
#if PHP_MAJOR_VERSION >= 8
  php_driver_default_session_handlers.compare =
      php_driver_default_session_compare;
#else
  php_driver_default_session_handlers.compare_objects =
      php_driver_default_session_compare;
#endif
