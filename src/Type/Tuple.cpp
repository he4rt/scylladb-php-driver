...
int php_driver_type_tuple_add(php_driver_type* type, zval* zsub_type) {
  ...
  if (cass_data_type_add_sub_type(type->data_type, sub_type->data_type) != CASS_OK) {
    return 0;
  }
  PHP5TO7_ZEND_HASH_NEXT_INDEX_INSERT(&type->data.tuple.types, zsub_type, sizeof(zval*));
  return 1;
}
...
PHP_METHOD(TypeTuple, __construct) {
  zend_throw_exception_ex(php_driver_logic_exception_ce, 0,
                          ...
                          "\\Type\\Tuple type is not supported.");
  return;
}
  ...
  if (argc > 0) {
    ...
    for (i = 0; i < argc; i++) {
      ...
      if (!PHP5TO7_ZEND_HASH_INDEX_FIND(&self->data.tuple.types, i, sub_type) ||
          !php_driver_validate_object(&args[i], sub_type)) {
        return;
      }
        ...
        PHP_ME(TypeTuple, name, arginfo_none, ZEND_ACC_PUBLIC)
            PHP_ME(TypeTuple, __toString, arginfo_tostring, ZEND_ACC_PUBLIC)
                PHP_ME(TypeTuple, types, arginfo_none, ZEND_ACC_PUBLIC)
                    PHP_ME(TypeTuple, create, arginfo_values, ZEND_ACC_PUBLIC) PHP_FE_END};

static zend_object_handlers php_driver_type_tuple_handlers;

static HashTable* php_driver_type_tuple_gc(
#if PHP_MAJOR_VERSION >= 8
    zend_object* object,
#else
    zendObject* object,
...
static HashTable* php_driver_type_tuple_properties(
#if PHP_MAJOR_VERSION >= 8
    zend_object* object
#else
    zendObject* object
  ...
  PHP5TO7_ZEND_HASH_UPDATE(props, "types", sizeof("types"), &types, sizeof(zval));

  return props;
}

static int php_driver_type_tuple_compare(zval* obj1, zval* obj2) {
#if PHP_MAJOR_VERSION >= 8
...
static void php_driver_type_tuple_free(zend_object* object) {
  php_driver_type* self = PHP5TO7_ZEND_OBJECT_GET(type, object);
...
static zend_object* php_driver_type_tuple_new(zend_class_entry* ce) {
...
void php_driver_define_TypeTuple() {
  ...
  memcpy(&php_driver_type_tuple_handlers, zend_get_std_object_handlers(),
