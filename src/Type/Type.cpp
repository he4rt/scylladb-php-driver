...
PHP_METHOD(Type, collection) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "O", &value_type, php_driver_type_ce) == FAILURE) {
    return;
  }
...
PHP_METHOD(Type, userType) {
  ...
  for (i = 0; i < argc; i += 2) {
    zval *name = &args[i];
    zval *sub_type = &args[i + 1];
    if (php_driver_type_user_type_add(type, Z_STRVAL_P(name), Z_STRLEN_P(name), sub_type)) {
      Z_ADDREF_P(sub_type);
    } else {
      break;
    }
...
PHP_METHOD(Type, map) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "OO", &key_type, php_driver_type_ce, &value_type,
                            php_driver_type_ce) == FAILURE) {
    return;
  }
...
static zend_function_entry php_driver_type_methods[] = {
    PHP_ABSTRACT_ME(Type, name, arginfo_none) PHP_ABSTRACT_ME(Type, __toString, arginfo_tostring)

#define XX_SCALAR_METHOD(name, _) \
  PHP_ME(Type, name, arginfo_none, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC | ZEND_ACC_FINAL)
        PHP_DRIVER_SCALAR_TYPES_MAP(XX_SCALAR_METHOD)
#undef XX_SCALAR_METHOD
            PHP_ME(Type, collection, arginfo_type,
                ...
                PHP_ME(Type, set, arginfo_type, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC | ZEND_ACC_FINAL)
                    PHP_ME(Type, map, arginfo_map,
                           ZEND_ACC_PUBLIC | ZEND_ACC_STATIC | ZEND_ACC_FINAL)
                        PHP_ME(Type, tuple, arginfo_types,
                               ZEND_ACC_PUBLIC | ZEND_ACC_STATIC | ZEND_ACC_FINAL)
                            PHP_ME(Type, userType, arginfo_types,
                                   ZEND_ACC_PUBLIC | ZEND_ACC_STATIC | ZEND_ACC_FINAL) PHP_FE_END};
