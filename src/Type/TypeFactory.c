...
#undef TYPE_INIT_METHOD
  ...
  ZEND_PARSE_PARAMETERS_END();
    ...
    php_scylladb_timestamp *timestamp = php_scylladb_timestamp_instantiate(return_value);
  ...
  if (self->type == CASS_VALUE_TYPE_TIME) {
    ...
    php_scylladb_time *time = php_scylladb_time_instantiate(return_value);
  ...
  if (self->type == CASS_VALUE_TYPE_DATE) {
    ...
    php_scylladb_date *date = php_scylladb_date_instantiate(return_value);
...
static struct node_s* php_scylladb_parse_node_new() {
  struct node_s* node;
  node = (struct node_s*)emalloc(sizeof(struct node_s));
  node->parent = nullptr;
  node->name = nullptr;
  node->name_length = 0;
