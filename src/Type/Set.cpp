...
PHP_METHOD(TypeSet, __construct) {
  zend_throw_exception_ex(php_driver_logic_exception_ce, 0,
                          ...
                          "\\Type\\Set type is not supported.");
  return;
}
        ...
        PHP_ME(TypeSet, name, arginfo_none, ZEND_ACC_PUBLIC)
            PHP_ME(TypeSet, valueType, arginfo_none, ZEND_ACC_PUBLIC)
                PHP_ME(TypeSet, __toString, arginfo_tostring, ZEND_ACC_PUBLIC)
                    PHP_ME(TypeSet, create, arginfo_value, ZEND_ACC_PUBLIC) PHP_FE_END};

static zend_object_handlers php_driver_type_set_handlers;

static HashTable *php_driver_type_set_gc(
#if PHP_MAJOR_VERSION >= 8
    zend_object *object,
#else
    zendObject *object,
...
static HashTable *php_driver_type_set_properties(
#if PHP_MAJOR_VERSION >= 8
    zend_object *object
#else
    zendObject *object
  ...
  PHP5TO7_ZEND_HASH_UPDATE(props, "valueType", sizeof("valueType"), &self->data.set.value_type,
                           sizeof(zval));
...
static int php_driver_type_set_compare(zval *obj1, zval *obj2) {
#if PHP_MAJOR_VERSION >= 8
...
static void php_driver_type_set_free(zend_object *object) {
  php_driver_type *self = PHP5TO7_ZEND_OBJECT_GET(type, object);
...
static zend_object *php_driver_type_set_new(zend_class_entry *ce) {
...
void php_driver_define_TypeSet() {
  ...
  memcpy(&php_driver_type_set_handlers, zend_get_std_object_handlers(),
