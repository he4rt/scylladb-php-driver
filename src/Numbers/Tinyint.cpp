  ...
  if (Z_TYPE_P(value) == IS_OBJECT &&
    ...
    if (Z_TYPE_P(value) == IS_LONG) {
      ...
      if (!php_driver_parse_int(Z_STRVAL_P(value), Z_STRLEN_P(value), &number)) {
        ...
        if (errno == ERANGE) {
          zend_throw_exception_ex(php_driver_range_exception_ce, 0,
                                  "value must be between -128 and 127, %s given",
                                  Z_STRVAL_P(value));
        }
        return;
      }
      ...
      INVALID_ARGUMENT(value,
                       "a long, a double, a numeric string or a " PHP_DRIVER_NAMESPACE "\\Tinyint");
    }
...
PHP_METHOD(Tinyint, __construct) { php_driver_tinyint_init(INTERNAL_FUNCTION_PARAM_PASSTHRU); }
/* }}} */
...
PHP_METHOD(Tinyint, abs) {
  ...
  object_init_ex(return_value, php_driver_tinyint_ce);
  result = PHP_DRIVER_GET_NUMERIC(return_value);
  result->data.tinyint.value =
      self->data.tinyint.value < 0 ? -self->data.tinyint.value : self->data.tinyint.value;
}
/* }}} */
                    ...
                    PHP_ME(Tinyint, add, arginfo_num, ZEND_ACC_PUBLIC)
                        PHP_ME(Tinyint, sub, arginfo_num, ZEND_ACC_PUBLIC)
                            PHP_ME(Tinyint, mul, arginfo_num, ZEND_ACC_PUBLIC)
                                PHP_ME(Tinyint, div, arginfo_num, ZEND_ACC_PUBLIC) PHP_ME(
                                    Tinyint, mod, arginfo_num, ZEND_ACC_PUBLIC)
                                    PHP_ME(Tinyint, abs, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                        Tinyint, neg, arginfo_none, ZEND_ACC_PUBLIC)
                                        PHP_ME(Tinyint, sqrt, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                            Tinyint, toInt, arginfo_none, ZEND_ACC_PUBLIC)
                                            PHP_ME(Tinyint, toDouble, arginfo_none, ZEND_ACC_PUBLIC)
                                                PHP_ME(Tinyint, min, arginfo_none,
                                                       ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
                                                    PHP_ME(Tinyint, max, arginfo_none,
                                                           ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
                                                        PHP_FE_END};
...
void php_driver_define_Tinyint() {
  ...
  memcpy(&php_driver_tinyint_handlers, zend_get_std_object_handlers(),
