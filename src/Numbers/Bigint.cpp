...
void php_driver_bigint_init(INTERNAL_FUNCTION_PARAMETERS) {
  ...
  if (Z_TYPE_P(value) == IS_LONG) {
    ...
    INVALID_ARGUMENT(value,
                     "a long, a double, a numeric string or a " PHP_DRIVER_NAMESPACE "\\Bigint");
  }
}

/* {{{ Bigint::__construct(string) */
PHP_METHOD(Bigint, __construct) { php_driver_bigint_init(INTERNAL_FUNCTION_PARAM_PASSTHRU); }
/* }}} */
...
PHP_METHOD(Bigint, abs) {
  ...
  object_init_ex(return_value, php_driver_bigint_ce);
  result = PHP_DRIVER_GET_NUMERIC(return_value);
  result->data.bigint.value =
      self->data.bigint.value < 0 ? -self->data.bigint.value : self->data.bigint.value;
}
/* }}} */
                    ...
                    PHP_ME(Bigint, add, arginfo_num, ZEND_ACC_PUBLIC)
                        PHP_ME(Bigint, sub, arginfo_num, ZEND_ACC_PUBLIC)
                            PHP_ME(Bigint, mul, arginfo_num, ZEND_ACC_PUBLIC)
                                PHP_ME(Bigint, div, arginfo_num, ZEND_ACC_PUBLIC) PHP_ME(
                                    Bigint, mod, arginfo_num, ZEND_ACC_PUBLIC)
                                    PHP_ME(Bigint, abs, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                        Bigint, neg, arginfo_none, ZEND_ACC_PUBLIC)
                                        PHP_ME(Bigint, sqrt, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                            Bigint, toInt, arginfo_none, ZEND_ACC_PUBLIC)
                                            PHP_ME(Bigint, toDouble, arginfo_none, ZEND_ACC_PUBLIC)
                                                PHP_ME(Bigint, min, arginfo_none,
                                                       ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
                                                    PHP_ME(Bigint, max, arginfo_none,
                                                           ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
                                                        PHP_FE_END};

static zend_object_handlers php_driver_bigint_handlers;
...
void php_driver_define_Bigint() {
  ...
  php_driver_bigint_handlers.get_properties = php_driver_bigint_properties;
  php_driver_bigint_handlers.get_gc = php_driver_bigint_gc;
  php_driver_bigint_handlers.compare = php_driver_bigint_compare;
  php_driver_bigint_handlers.cast_object = static_cast<zend_object_cast_t>(php_driver_bigint_cast);
  php_driver_bigint_handlers.clone_obj = NULL;
}
END_EXTERN_C()