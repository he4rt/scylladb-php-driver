...
static zend_result to_string(zval *result, php_driver_numeric *flt) {
  char *string;
...
void php_driver_float_init(INTERNAL_FUNCTION_PARAMETERS) {
  ...
  if (Z_TYPE_P(value) == IS_LONG) {
    ...
    if (!php_driver_parse_float(Z_STRVAL_P(value), Z_STRLEN_P(value), &self->data.floating.value)) {
      return;
    }
    ...
    INVALID_ARGUMENT(
        value, "a long, double, numeric string or a " PHP_DRIVER_NAMESPACE "\\Float instance");
  }
}

/* {{{ Float::__construct(string) */
PHP_METHOD(Float, __construct) { php_driver_float_init(INTERNAL_FUNCTION_PARAM_PASSTHRU); }
/* }}} */
...
static zend_function_entry php_driver_float_methods[] = {
    PHP_ME(Float, __construct, arginfo__construct, ZEND_ACC_CTOR | ZEND_ACC_PUBLIC) PHP_ME(
        Float, __toString, arginfo_tostring, ZEND_ACC_PUBLIC)
        PHP_ME(Float, type, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
            Float, value, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(Float, isInfinite, arginfo_none,
                                                                ZEND_ACC_PUBLIC)
            PHP_ME(Float, isFinite, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                Float, isNaN, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(Float, add, arginfo_num,
                                                                    ZEND_ACC_PUBLIC)
                PHP_ME(Float, sub, arginfo_num, ZEND_ACC_PUBLIC) PHP_ME(
                    Float, mul, arginfo_num, ZEND_ACC_PUBLIC) PHP_ME(Float, div, arginfo_num,
                                                                     ZEND_ACC_PUBLIC)
                    PHP_ME(Float, mod, arginfo_num, ZEND_ACC_PUBLIC)
                        PHP_ME(Float, abs, arginfo_none, ZEND_ACC_PUBLIC)
                            PHP_ME(Float, neg, arginfo_none, ZEND_ACC_PUBLIC)
                                PHP_ME(Float, sqrt, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                    Float, toInt, arginfo_none, ZEND_ACC_PUBLIC)
                                    PHP_ME(Float, toDouble, arginfo_none, ZEND_ACC_PUBLIC) PHP_ME(
                                        Float, min, arginfo_none, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
                                        PHP_ME(Float, max, arginfo_none,
                                               ZEND_ACC_PUBLIC | ZEND_ACC_STATIC) PHP_FE_END};

static zend_object_handlers php_driver_float_handlers;

static HashTable *php_driver_float_gc(
...
static HashTable *php_driver_float_properties(
#if PHP_MAJOR_VERSION >= 8
    zend_object *object
#else
    zendObject *object
...
static inline cass_int32_t float_to_bits(cass_float_t value) {
  cass_int32_t bits;
...
static int php_driver_float_compare(zval *obj1, zval *obj2) {
#if PHP_MAJOR_VERSION >= 8
...
static
    ...
    int
#endif
    php_driver_float_cast(zend_object *object, zval *retval, int type) {
#if PHP_MAJOR_VERSION >= 8
  php_driver_numeric *self = PHP5TO7_ZEND_OBJECT_GET(numeric, object);
#else
  php_driver_numeric *self = PHP_DRIVER_GET_NUMERIC(object);
...
static void php_driver_float_free(zend_object *object) {
  php_driver_numeric *self = PHP5TO7_ZEND_OBJECT_GET(numeric, object);

  zend_object_std_dtor(&self->zendObject);
}

static zend_object *php_driver_float_new(zend_class_entry *ce) {
...
void php_driver_define_Float() {
  ...
  php_driver_float_handlers.get_properties = php_driver_float_properties;
  php_driver_float_handlers.get_gc = php_driver_float_gc;
  php_driver_float_handlers.compare = php_driver_float_compare;
  php_driver_float_handlers.cast_object = php_driver_float_cast;
  php_driver_float_handlers.clone_obj = NULL;
}
END_EXTERN_C()