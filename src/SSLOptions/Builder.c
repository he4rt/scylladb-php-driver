...
ZEND_METHOD(Cassandra_SSLOptions_Builder, build) {
  ...
  cass_ssl_set_verify_flags(ssl->ssl, (int)(builder->flags));
...
void php_scylladb_ssl_options_builder_free(zend_object *object) {
  const php_scylladb_ssl_builder *self = PHP_SCYLLADB_OBJ_FETCH(php_scylladb_ssl_builder, object);
...
zend_object *php_scylladb_ssl_options_builder_new(zend_class_entry *ce) {
  php_scylladb_ssl_builder *self = PHP_SCYLLADB_OBJ_ALLOCATE(php_scylladb_ssl_builder, ce, &php_scylladb_ssl_options_builder_handlers);
